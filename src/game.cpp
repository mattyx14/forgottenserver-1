...
ReturnValue Game::internalMoveItem(Cylinder* fromCylinder, Cylinder* toCylinder, int32_t index, Item* item, uint32_t count, Item** _moveItem, uint32_t flags /*= 0*/, Creature* actor/* = nullptr*/)
{
	if (!toCylinder) {
	...
	if (ret != RET_NOERROR) {
		if (ret == RET_TOOFARAWAY) {
			...
			if (getPathToEx(player, walkToPos, listDir, 0, 1, true, true)) {
				g_dispatcher.addTask(createTask(boost::bind(&Game::playerAutoWalk, this, player->getID(), listDir)));
	...
	if (tradePartner->getTradeState() == TRADE_ACCEPT) {
		...
		if (ret1 == RET_NOERROR && ret2 == RET_NOERROR) {
			...
			if (ret1 == RET_NOERROR && ret2 == RET_NOERROR) {
				...
				ret1 = internalMoveTradeItem(cylinder1, tradePartner, INDEX_WHEREEVER, tradeItem1, tradeItem2, count1, nullptr, FLAG_IGNOREAUTOSTACK);
				if (ret1 == RET_NOERROR) {
...
bool Game::combatChangeHealth(CombatType_t combatType, Creature* attacker, Creature* target, int32_t healthChange)
{
	...
	if (healthChange > 0) {
		...
		if (g_config.getBoolean(ConfigManager::SHOW_HEALTH_CHANGE) && realHealthChange > 0 && !target->isInGhostMode()) {
			SpectatorVec list;
			getSpectators(list, targetPos, false, true);
...
bool Game::combatChangeMana(Creature* attacker, Creature* target, int32_t manaChange)
{
	if (manaChange > 0) {
		...
		addAnimatedText(targetPos, TEXTCOLOR_BLUE, buffer);
	}
